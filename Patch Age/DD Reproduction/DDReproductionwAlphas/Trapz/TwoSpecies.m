function [n,m,SI,SJ] = TwoSpecies(Na,Nt,amax,tmax,InitCon1,InitCon2,Birth1,Death1,Birth2,Death2,gamma,alpha1,alpha2)
...
%rho = RhoHRAge(Na,Nt,amax, tmax,gamma);

for k = 1:length(t)-2
   %% Gathers model functions for current time step 
   RepoN = ReproductionFunction1(a,n(:,k),m(:,k),rho(:,k),b1,A1,A2);
   RepoM = ReproductionFunction2(a,m(:,k),n(:,k),rho(:,k),b2,A2,A1);

   DeathN = DeathFunction1(a,n(:,k),m(:,k),rho(:,k),mu1);
   DeathM = DeathFunction2(a,m(:,k),n(:,k),rho(:,k),mu2);
 ...
 %% Repeat calculations using half time information
   ...
   RepoN = ReproductionFunction1(a,n(:,s),m(:,s),rho(:,s),b1,A1,A2);
   RepoM = ReproductionFunction2(a,m(:,s),n(:,s),rho(:,s),b2,A2,A1);
   
   DeathN = DeathFunction1(a,n(:,s),m(:,s),rho(:,s),mu1);
   DeathM = DeathFunction2(a,m(:,s),n(:,s),rho(:,s),mu2);
...
SN = ReproductionFunction1(a,n(:,end),m(:,end),rho(:,end),b1,A1,A2);
SM = ReproductionFunction2(a,m(:,end),n(:,end),rho(:,end),b2,A2,A1);
SI = SN(1);
SJ = SM(1);

%% Calculate Estimated reproduction numbers (currently unused)
RepoNum1 = max(0,1-A1*SJ/mu2)*SingleSpeciesR1;
RepoNum2 = max(0,1-A2*SI/mu1)*SingleSpeciesR2;
