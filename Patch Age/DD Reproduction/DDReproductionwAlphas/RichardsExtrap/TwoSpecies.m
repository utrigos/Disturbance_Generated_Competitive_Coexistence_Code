...
for k = 1:length(t)-2
 %% Gathers model functions for current time step 
   ...
   RepoM = ReproductionFunction2(a,m(:,k),n(:,k),rho(:,k),b2,A1,A2);
 ...
 %% Calculate half time boundary value
  ...
  %% Repeat calculations using half time information
   ...
   RepoM = ReproductionFunction2(a,m(:,s),n(:,s),rho(:,s),b2,A1,A2);
...
SM = ReproductionFunction2(a,m(:,end),n(:,end),rho(:,end),b2,A1,A2);
SI = SN(1);
SJ = SM(1);
