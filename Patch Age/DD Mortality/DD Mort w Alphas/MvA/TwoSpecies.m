...
%% Gathers model functions for current time step 
   RepoN = ReproductionFunction1(a,n(:,k),m(:,k),rho(:,k),b1);
   RepoM = ReproductionFunction2(a,m(:,k),n(:,k),rho(:,k),b2);

   DeathN = DeathFunction1(a,n(:,k),m(:,k),rho(:,k),mu1,a1,a2);
   DeathM = DeathFunction2(a,m(:,k),n(:,k),rho(:,k),mu2,a1,a2);
 ...
 %% Calculate half time boundary value
   ...
   RepoN = ReproductionFunction1(a,n(:,s),m(:,s),rho(:,s),b1);
   RepoM = ReproductionFunction2(a,m(:,s),n(:,s),rho(:,s),b2);
   
   DeathN = DeathFunction1(a,n(:,s),m(:,s),rho(:,s),mu1,a1,a2);
   DeathM = DeathFunction2(a,m(:,s),n(:,s),rho(:,s),mu2,a1,a2);
...
SN = ReproductionFunction1(a,n(:,end),m(:,end),rho(:,end),b1);
SM = ReproductionFunction2(a,m(:,end),n(:,end),rho(:,end),b2);
SI = SN(1);
SJ = SM(1);
